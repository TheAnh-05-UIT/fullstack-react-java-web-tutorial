          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="flex items-center gap-4 mb-4">
                <div className="w-14 h-14 bg-white/20 backdrop-blur-sm border border-white/30 rounded-2xl flex items-center justify-center">
                  <Icon className="w-7 h-7 text-white" />
                </div>
                <h1 className="text-5xl sm:text-6xl font-black tracking-tight">{data.name}</h1>
              </div>
              <p className="text-lg sm:text-xl text-white/85 leading-relaxed max-w-xl">{data.tagline}</p>
            </div>

            {/* 8-Stage Lifecycle Stepper */}
            <div className="pt-2">
              <div className="text-xs font-bold uppercase tracking-wider text-white/80 mb-3.5 flex items-center gap-1.5">
                ...
                {LIFECYCLE_STAGES.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group relative flex items-center gap-2.5 px-3 py-2.5 rounded-xl border font-mono transition-all duration-300 text-left cursor-pointer ${
                        isActive
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className={`text-xs truncate leading-tight mt-0.5 ${isActive ? 'font-black text-slate-900' : 'font-semibold text-white/90'}`}>
                          {s.label}
                        </div>
                      </div>
                      {isActive && (
